...
当前 `npm run test` 使用 Node 的 TypeScript 测试运行器，只执行 `src/**/*.test.ts`，适合纯函数、解析、状态和公共基础能力测试。它不提供 DOM 渲染、浏览器交互或 E2E 环境。
...
- 新增或修改测试时，运行 `npm run test`；涉及类型、lint 或构建规则时同时运行 `npm run lint` 与 `npm run build`。
...
5. 是否执行并报告 `npm run test`，以及改动范围需要的 lint/build？
