...
对于已确认不再承担设计母版职责、且长期静态展示的大型位图，开发者可显式执行 `pnpm run assets:optimize`。该源码迁移工具只检查 `src/**/assets/**/*.png`：仅当文件至少为 512 KiB、不是 APNG、具有可解析静态引用且 WebP 更小时，才创建同目录 WebP、更新引用并删除 PNG。它不处理 `public/`、动态路径、小图、GIF、SVG 或既有 WebP，也不在 `build`、CI 或 Vite plugin 生命周期中运行。运行后必须人工检查清晰度、透明边缘和 Git diff，再将迁移结果作为普通资源改动提交；设计母版应保留在设计工具或明确的非发布归档中。
...
每个有公共边界的 FSD slice、`app` 技术 segment、共享能力和共享 UI 都必须在模块根目录维护 `README.md`；它服务于新成员和新 Codex 上下文的任务定位，不是实现注释的副本。单文件能力的现有例外是 `shared/lib/format.ts`，其说明为相邻 `format.md`。

说明至少写明：职责与非职责、外部入口、关键约束、扩展位置和验证方式。`ui`、`model`、`api` 等仅承载同一模块内部实现的子目录不单独要求 README，资源目录和空目录也不创建假文档。新增模块时必须随实现创建说明；`pnpm run lint:module-docs` 会验证受管范围内的说明文件存在且有标题。
