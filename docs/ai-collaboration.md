...
| 测试基础设施     | `docs/testing-standards.md`、`vitest.config.ts`、`tsconfig.test.json`                                          | `.test.ts` 保持 Node 环境，`.test.tsx` 使用 Vitest/jsdom；新增测试就近放置并通过 `pnpm run test`，覆盖率按 `.ts`/`.tsx` 输出到两个目录。E2E 与网络 Mock Server 需有真实需求后单独接入。                                                                                                   |
...
| 实时连接         | `src/shared/socket`                                                                                            | 统一 Ticket、Socket.IO 握手、消息解析、连接状态和重连；业务模块不得直接导入 `socket.io-client`，不得把 PK 事件放入共享层。                                                                                                                                                                |
| 设备能力         | `src/shared/lib/device`                                                                                        | 统一 PC、平板、H5 和浏览器环境判断；布局由 CSS 断点负责，诊断原始数据不携带页面文案。                                                                                                                                                                                                     |
| 位置能力         | `src/shared/lib/location`                                                                                      | 统一浏览器定位、坐标校验和浏览器原始坐标/GCJ-02 选择；默认返回浏览器坐标，业务需要国标坐标时显式传 `coordinateSystem: 'gcj02'`，不复制转换算法或权限错误处理。                                                                                                                            |
| 高德地图         | `src/shared/lib/amap`、`src/shared/ui/tld-amap-picker`                                                         | 统一 JS API 按需加载、逆地址解析、地图选点和导航 URI；地图坐标使用 GCJ-02，业务不得直接导入 Loader 或读取 `window.AMap`。公开 Web Key/安全密钥通过环境模板配置并限制部署域名。                                                                                                            |
| 图表组件         | `src/shared/ui/tld-echart`                                                                                     | 统一 ECharts 按需注册、实例生命周期、尺寸响应和可访问名称；页面只传递类型化 option，不直接操作 ECharts 实例。                                                                                                                                                                             |
...
| 文件下载         | `src/shared/lib/download`                                                                                      | 调用方负责内容、MIME、文件名和权限；工具通过 `download` 属性触发下载，并将 PDF/TXT 等可内联 Blob MIME 转为 `application/octet-stream`。跨域 URL 需服务端提供 `Content-Disposition: attachment` 或由调用方先获取 Blob。                                                                    |
...
| 样式与响应式     | `src/app/styles`、各模块 `*.module.scss`                                                                       | Web 端验收 PC、平板、H5；使用 CSS-first 断点、token、流式容器和模块样式，1279px 及以下折叠侧栏、1280px 起恢复完整桌面侧栏，禁止 rem 整体缩放、设备类型驱动整页布局或新增全局业务样式。                                                                                                    |
...
- 测试按模块能力和可验证行为就近放置，禁止按页面数量机械创建。公式、解析、状态、公共能力和缺陷修复必须补同目录 `.test.ts`；真实 React、Hook、Context、Effect 或 DOM 交互使用 `.test.tsx`。两类测试都由 `pnpm run test` 执行，E2E 在稳定跨页面流程出现后再单独接入，详见 [测试规范](./testing-standards.md)。
...
- Web 端响应式采用 SCSS + CSS Modules，常规尺寸使用 px，连续尺寸少量使用 clamp；不使用根字号缩放或 px 自动转换。断点为 H5 `<768px`、平板 `768px–1023px`、桌面 `>=1024px`，1279px 及以下折叠侧栏、1280px 起恢复完整桌面侧栏，Electron 仍只承诺桌面端。
