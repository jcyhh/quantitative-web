...
const sourceExtensions = new Set(['.css', '.html', '.js', '.jsx', '.mjs', '.scss', '.ts', '.tsx', '.vue'])
...
async function createMigrationPlan(rootDirectory, fileSystem) {
    ...
    const candidates = (await walkFiles(sourceDirectory, fileSystem)).filter(
        (filePath) => extname(filePath).toLowerCase() === '.png' && isManagedAsset(filePath, sourceDirectory),
    )
    const skipped = []
    const migrations = []
    ...
    try {
        ...
        await Promise.all(
            committedSourceFiles.map((sourceFile) =>
                fileSystem.writeFile(sourceFile.path, sourceFile.originalContent, 'utf8').catch(() => undefined),
            ),
        )
        await Promise.all(
