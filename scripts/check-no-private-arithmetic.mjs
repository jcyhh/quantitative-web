...
const permittedRoots = [path.resolve('src/shared/lib/decimal'), path.resolve('src/shared/lib/number')]
...
if (violations.length > 0) {
    ...
    for (const violation of violations)
        console.error(
            `${violation.file}:${violation.line}:${violation.column} ${violation.operator} → use shared/lib/decimal for financial values, shared/lib/number for non-financial values, or a template literal for text concatenation.`,
        )
    process.exitCode = 1
}
